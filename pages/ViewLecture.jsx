    ...
    <div className='min-h-screen bg-gray-50 p-6 flex flex-col md:flex-row gap-6'>
      {console.log(selectedCourse)}
         ...
         <div className='mb-6'>
            <h2 className='text-2xl font-bold flex items-center justify-start gap-[20px] text-gray-800 '>
                 <FaArrowLeftLong className='text-black w-[22px] h-[22px] cursor-pointer'
                  onClick={()=>navigate(`/viewcourse/${courseId}`)}/>{selectedCourse?.title}</h2>
       ...
       <div className='aspect-video bg-black rounded-xl overflow-hidden mb-4 border border-gray-300 '>
        ...
        <div className='flex items-center justify-center h-full text-white'> Selecet a lecture to start watching </div>}
       </div>
       <div className='mt-2'>
        <h2 className='text-lx font-semibold text-gray-800'>{selectedLecture?.lectureTitle}</h2>
       </div>
        </div>
       ...
       <button key={index} onClick={()=>setSelectedLecture(lecture)} className= {`flex items-center 
       justify-between p-3 rounded-lg border transition text-left ${
        selectedLecture?. _id === lecture._id? 'bg-gray-200 border-gray-500': 'hover:bg-gray-50 border-gray-300'
       }`}>
       <h2 className='text-sm font-semibold text-gray-800'> {lecture.lectureTitle}</h2> < FaPlayCircle className='text-lg text-black'/>
        ...
        <div className='mt-4 border-t pt-4'>
          ...
          <div className='flex items-center gap-4 '>
            <img src={creatorData?.photoUrl} alt="" className='w-14 h-14 rounded-full object-cover' /></div>
            <div>
