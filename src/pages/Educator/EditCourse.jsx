...
function EditCourse() {
  ...
  useEffect(() => {
    if (selectCourse) {
      ...
      setSubTitle(selectCourse.subtitle || ""); // lowercase to match backend
  ...
  const handleRemoveCourse = async () => {
    setLoading1(true);
    try {
      await axios.delete(serverUrl + `/api/course/deletecourse/${courseId}`, { withCredentials: true });
    ...
    <div className='max-w-5xl mx-auto p-6 mt-10 bg-white rounded-lg shadow-md'>
      ...
      <div className='flex items-center justify-center gap-[20px] md:justify-between flex-col md:flex-row mb-6 relative'>
        ...
        <div className='space-x-2 space-y-2'>
          ...
          }} className='bg-black text-white px-4 py-2 rounded-md'>Go to Lecture page</button>
        </div>
      </div>
      ...
      <div className='bg-gray-50 p-6 rounded-md'>
        ...
        <div className='space-x-2 space-y-2'>
          ...
          >
            {loading1 ? <ClipLoader size={25} color='white' /> : "Remove Course"}
          </button>
        </div>
